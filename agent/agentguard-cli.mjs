 ...
 *   AGENTGUARD_URL=http://localhost:3000 node agent/agentguard-cli.mjs 0x...
 *
...
const AGENTGUARD_URL = process.env.AGENTGUARD_URL ?? "https://agentguard402.vercel.app";
...
async function main() {
  ...
  console.log("");
  console.log(color("AgentGuard402 demo agent", "bold"));
  ...
  console.log(color(`endpoint: ${AGENTGUARD_URL}/api/preflight`, "gray"));
  console.log("");
  ...
  let res;
  try {
    res = await fetch(`${AGENTGUARD_URL}/api/preflight`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-payment": "agentguard-cli-demo-payment-payload",
        "user-agent": "agentguard402-cli/0.1.0",
      },
      body: JSON.stringify({
        chain: CHAIN,
